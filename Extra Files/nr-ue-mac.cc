...
// --------------------------------------------------------
uint32_t TotHolDelay1=0;
uint32_t count1=0;
uint32_t TotHolDelay2=0;
uint32_t count2=0;
uint32_t TotHolDelay3=0;
uint32_t count3=0;
uint32_t TotHolDelay4=0;
uint32_t count4=0;
uint32_t TotHolDelay5=0;
uint32_t count5=0;
// --------------------------------------------------------
...
NrUeMac::DoReportBufferStatus (LteMacSapProvider::ReportBufferStatusParameters params)
{
  ...
  // --------------------------------------------------------
  if (params.rnti == 1) {
    TotHolDelay1 += params.txQueueHolDelay;
    count1 = count1 + 1;
    double meanDel = (double)TotHolDelay1 / count1;
    NS_LOG_INFO("Queue Hol Del for UE 1 (ms): " << static_cast<uint16_t>(params.txQueueHolDelay));
    NS_LOG_INFO("Total Queue Hol Del till now for UE 1 (ms): " << TotHolDelay1);
    NS_LOG_INFO("Mean Queue Hol Del till now for UE 1 (ms): " << meanDel);
  } else if (params.rnti == 2) {
    TotHolDelay2 += params.txQueueHolDelay;
    count2 = count2 + 1;
    double meanDel = (double)TotHolDelay2 / count2;
    NS_LOG_INFO("Queue Hol Del for UE 2 (ms): " << static_cast<uint16_t>(params.txQueueHolDelay));
    NS_LOG_INFO("Total Queue Hol Del till now for UE 2 (ms): " << TotHolDelay2);
    NS_LOG_INFO("Mean Queue Hol Del till now for UE 2 (ms): " << meanDel);
  } else if (params.rnti == 3) {
    TotHolDelay3 += params.txQueueHolDelay;
    count3 = count3 + 1;
    double meanDel = (double)TotHolDelay3 / count3;
    NS_LOG_INFO("Queue Hol Del for UE 3 (ms): " << static_cast<uint16_t>(params.txQueueHolDelay));
    NS_LOG_INFO("Total Queue Hol Del till now for UE 3 (ms): " << TotHolDelay3);
    NS_LOG_INFO("Mean Queue Hol Del till now for UE 3 (ms): " << meanDel);
  } else if (params.rnti == 4) {
    TotHolDelay4 += params.txQueueHolDelay;
    count4 = count4 + 1;
    double meanDel = (double)TotHolDelay4 / count4;
    NS_LOG_INFO("Queue Hol Del for UE 4 (ms): " << static_cast<uint16_t>(params.txQueueHolDelay));
    NS_LOG_INFO("Total Queue Hol Del till now for UE 4 (ms): " << TotHolDelay4);
    NS_LOG_INFO("Mean Queue Hol Del till now for UE 4 (ms): " << meanDel);
  } else if (params.rnti == 5) {
    TotHolDelay5 += params.txQueueHolDelay;
    count5 = count5 + 1;
    double meanDel = (double)TotHolDelay5 / count5;
    NS_LOG_INFO("Queue Hol Del for UE 5 (ms): " << static_cast<uint16_t>(params.txQueueHolDelay));
    NS_LOG_INFO("Total Queue Hol Del till now for UE 5 (ms): " << TotHolDelay5);
    NS_LOG_INFO("Mean Queue Hol Del till now for UE 5 (ms): " << meanDel);
  }
