...
NS_OBJECT_ENSURE_REGISTERED (MyTdma);

TypeId
MyTdma::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MyTdma")
...
MyTdma::GetUeVectorFromActiveUeMap (const NrMacSchedulerNs3::ActiveUeMap &activeUes)
{
 ...
 * \see BeforeDlSched
 */
MyTdma::BeamSymbolMap
MyTdma::AssignRBGTDMA (uint32_t symAvail, const ActiveUeMap &activeUe,
      ...
      while (schedInfoIt != ueVector.end ())
        {
          ...
          // Here we store the UE RNTI type casted into a uint8_t currentRnti variable to use it further for shared memory transactions
          // Then we put both the values in the shared memory to be used by python side of our code. We do this with the help of SetBsr and SetTarget functions.
          // Then we set the predicted BSR value from the python side to receivedBufferSize variable and use it to update required metrics.
 ...
 * (e.g., the DL TBS, the DL RBG) and then will call MyTdma::AssignRBGTDMA.
 */
MyTdma::BeamSymbolMap
MyTdma::AssignDLRBG (uint32_t symAvail, const ActiveUeMap &activeDl) const
{
  NS_LOG_FUNCTION (this);

  BeforeSchedFn beforeSched = std::bind (&MyTdma::BeforeDlSched, this,
                                         std::placeholders::_1, std::placeholders::_2);
  AfterSuccessfullAssignmentFn SuccFn = std::bind (&MyTdma::AssignedDlResources, this,
                                                   std::placeholders::_1, std::placeholders::_2,
                                                   std::placeholders::_3);
  AfterUnsucessfullAssignmentFn UnSuccFn = std::bind (&MyTdma::NotAssignedDlResources, this,
                                                      std::placeholders::_1, std::placeholders::_2,
                                                      std::placeholders::_3);
  GetCompareUeFn compareFn = std::bind (&MyTdma::GetUeCompareDlFn, this);
 ...
 * (e.g., the UL TBS, the UL RBG) and then will call MyTdma::AssignRBGTDMA.
 */
MyTdma::BeamSymbolMap
MyTdma::AssignULRBG (uint32_t symAvail, const ActiveUeMap &activeUl) const
{
  NS_LOG_FUNCTION (this);
  BeforeSchedFn beforeSched = std::bind (&MyTdma::BeforeUlSched, this,
                                         std::placeholders::_1, std::placeholders::_2);
  AfterSuccessfullAssignmentFn SuccFn = std::bind (&MyTdma::AssignedUlResources, this,
                                                   std::placeholders::_1, std::placeholders::_2,
                                                   std::placeholders::_3);
  GetCompareUeFn compareFn = std::bind (&MyTdma::GetUeCompareUlFn, this);
  AfterUnsucessfullAssignmentFn UnSuccFn = std::bind (&MyTdma::NotAssignedUlResources, this,
                                                      std::placeholders::_1, std::placeholders::_2,
                                                      std::placeholders::_3);
...
MyTdma::CreateDlDci (PointInFTPlane *spoint,
 ...
 */
std::shared_ptr<DciInfoElementTdma>
MyTdma::CreateUlDci (NrMacSchedulerNs3::PointInFTPlane *spoint,
                                     const std::shared_ptr<NrMacSchedulerUeInfo> &ueInfo,
                                     uint32_t maxSym) const
{
  NS_LOG_FUNCTION (this);
 ...
 */
std::shared_ptr<DciInfoElementTdma>
MyTdma::CreateDci (NrMacSchedulerNs3::PointInFTPlane *spoint,
                               const std::shared_ptr<NrMacSchedulerUeInfo> &ueInfo,
                               ...
                               uint32_t mcs, uint8_t numSym) const
{
  ...
  return dci;
          ...
          // // -----------------------------------------------------------------------------------------------
          // if(GetUe (*schedInfoIt)->m_rnti==1){
          //     uint16_t temp = (uint16_t)bufQueueSize;
          //     NS_LOG_LOGIC ("BSR before pred " << temp << " reported");
          //     m_bsrDl->SetWbCQI (temp);
          //     temp = m_bsrDl->GetWbCQI ();
          //     schedInfoIt->second=temp;
          //     bufQueueSize=temp;
          //     NS_LOG_LOGIC ("BSR after pred " << temp << " reported");
          // }
          // // -----------------------------------------------------------------------------------------------
          ...
          // uint16_t temp = (uint16_t)bufQueueSize;
          // m_bsrDl->SetWbCQI (temp);
          // uint8_t temp2 = (uint8_t)GetUe (*schedInfoIt)->m_rnti;
          // m_bsrDl->SetTarget (temp2);
          // // -----------------------------------------------------------------------------------------------
